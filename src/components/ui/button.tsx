...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-full border border-transparent bg-clip-padding text-sm font-semibold whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-40 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        // KDS: kind="dominant" palette="brand" variant="fill"
        default:
          "bg-primary text-primary-foreground hover:bg-kroger-brand-most-prominent",
        // KDS: kind="dominant" palette="brand" variant="border"
        outline:
          "border border-primary bg-background text-primary hover:bg-kroger-accent-most-subtle aria-expanded:bg-kroger-accent-most-subtle dark:border-input dark:bg-input/30 dark:hover:bg-input/50",
        secondary:
          "bg-secondary text-secondary-foreground hover:bg-[color-mix(in_oklch,var(--secondary),var(--foreground)_5%)] aria-expanded:bg-secondary aria-expanded:text-secondary-foreground",
        // KDS: kind="dominant" palette="neutral" variant="minimal"
        ghost:
          "text-foreground hover:bg-muted aria-expanded:bg-muted aria-expanded:text-foreground dark:hover:bg-muted/50",
        // KDS: kind="dominant" palette="negative" variant="fill"
        destructive:
          "bg-destructive text-white hover:bg-kroger-negative-most-prominent",
